...
def diffusive(dim, levels, prior_range, N_level, N_iter, N_max, MC_step, C, C1, beta, lam, seed):
    ...
    while i < N_level:
        ...
        i += 1
    L_set.sort()
    L_level.append(L_set[floor(N_level*quantile)-1])
    L_quantile = L_set[floor(N_level*quantile)-1:]
    L_set = L_quantile

    j = 1
    n_tot = [[N_level, floor(N_level*quantile)-1]]
    while j < levels:
        i = 0
        k = 0
        #print('--------------------------------------------------------------------------------------------')
        ...
        L_set = L_quantile
        n_tot.append([N_level, floor(N_level*quantile)-1])
        j += 1
        if n_lk == N_max:
            break
    it_unif = n_lk
    i = n_lk*Q
    n_tot.append([N_level, floor(N_level*quantile)-1])
    X_swaths = [n_tot[0][1]/n_tot[0][0]]
    for i in range(1, len(n_tot)):
        X_swaths.append(n_tot[i][1]/n_tot[i][0]*X_swaths[-1])
    ...
    for like in L_recorded:
        ...
        if j == int(level_visits[i]):
            if i == len(L_level)-1:
                ...
                #X.append(X_swaths[i]*((X_swaths[i+1]/X_swaths[i])**(j/level_visits[i])-(X_swaths[i+2]/X_swaths[i+1])**(1/level_visits[i+1])))
        else:
...
if __name__ == '__main__':
    ...
    for param in params_diff:
        params = [(*param, i) for i in range(12)]
        start = time.time()
        with Pool() as pool:
